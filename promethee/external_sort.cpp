...
void ExternalSort::main(vector<string> args)
{
    ...
    this->input = TIFFOpen(this->path.c_str(), "r");
...
{
    this->input = TIFFOpen(this->positions.c_str(), "r");
    TIFFGetField(input, TIFFTAG_IMAGEWIDTH, &this->width);
    TIFFGetField(input, TIFFTAG_IMAGELENGTH, &this->height);
    TIFFGetField(input, TIFFTAG_SAMPLEFORMAT, &this->sampleFormat);
    this->output = TIFFOpen(this->path.c_str(), "r");
    ...
    TIFF *values = TIFFOpen(nameParcialValue.c_str(), "w8");
    ...
    TIFF *positions = TIFFOpen(nameParcialPositions.c_str(), "w8");
...
{
    this->input = TIFFOpen(this->path.c_str(), "w8");
    ...
    this->output = TIFFOpen(this->positions.c_str(), "w8");
    ...
    for (int i = 0; i < paths.size(); i++)
    {
        values[i] = TIFFOpen(paths[i].first.c_str(), "r");
        positions[i] = TIFFOpen(paths[i].second.c_str(), "r");
        pointersX[i] = 0;
        pointersY[i] = 0;
    ...
    while (!pq.empty())
    {
        ...
        if (pointersX[i] == (this->width * endBuffer[i]))
        {
            ...
            if (pointersY[i] < size[i])
            {
                ...
                pointersX[i] = this->width * endBuffer[i];
        }
    }
